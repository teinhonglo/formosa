...
dir=$exp_root/chain${chain_affix}/${tdnn_semi}_smbr
...
if [ $stage -le 8 ]; then
  steps/best_path_weights_smbr.sh --cmd "${train_cmd}" --acwt 0.1 \
    $data_root/${unsupervised_set_perturbed}_hires_comb \
    $sup_chain_dir/decode_${unsupervised_set_perturbed}_big \
    $sup_chain_dir/best_path_${unsupervised_set_perturbed}_smbr_big
fi
...
if [ $stage -le 10 ]; then
  steps/nnet3/chain/make_weighted_den_fst.sh --num-repeats $lm_weights --cmd "$train_cmd" \
    ${sup_tree_dir} ${sup_chain_dir}/best_path_${unsupervised_set_perturbed}_smbr_big \
    $dir
fi
...
if [ -z "$unsup_egs_dir" ]; then
  ...
  if [ $stage -le 13 ]; then
    ...
    steps/nnet3/chain/get_egs.sh \
      ...
      --deriv-weights-scp $sup_chain_dir/best_path_${unsupervised_set_perturbed}_smbr_big/weights.scp \
      --online-ivector-dir $ivector_root_dir/ivectors_${unsupervised_set_perturbed}_hires_comb \
...
if [ $stage -le 13 ]; then
  for test_set in test online/eval0 online/eval1; do
